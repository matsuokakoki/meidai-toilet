 ...
 */
export function calculateCongestion(toilet: ToiletData, hour: number, weather: string = 'Sunny'): number {
  let score :number= 0 // 基準となる初期混雑度（20%）

  let times: number = hour * 100; // 例: 12.45 なら 1245 になる
  ...
  if (!isNaN(tLat) && !isNaN(tLng) && tLat !== 0 && tLng !== 0) {
    ...
    for (const hotspot of HOTSPOTS) {
      ...
      if (hotspot.activeStartTime !== undefined && hotspot.activeEndTime !== undefined) {
        if (times < hotspot.activeStartTime || times > hotspot.activeEndTime) {
          continue; // 時間外ならスキップ
        }
      }

      const distance = getDistanceFromLatLonInM(hotspot.lat, hotspot.lng, tLat, tLng);
  
      if (distance <= hotspot.radiusM) {
        // そのホットスポットによるスコアを計算（中心に近いほど高い）
        const currentHotspotScore = hotspot.maxScore * (1 - (distance / hotspot.radiusM));
  ...
  score = timescore + Setubisore + LocationScore + WeatherScore + ReviewScore + floorscore;
...
export function getCongestionStatus(score: number) {
  if (score >= 80) return { text: '激混み', color: 'text-red-600', bg: 'bg-red-100' }
  if (score >= 50) return { text: 'やや混雑', color: 'text-orange-600', bg: 'bg-orange-100' }
  if (score === 0) return { text: '空き/閉館', color: 'text-blue-600', bg: 'bg-blue-100' } // 🌟 0%の時のテキストを微調整
